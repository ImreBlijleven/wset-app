...
let winesSubscription = null
 
async function initSupabaseSync() {
  // Subscribe to real-time changes (nieuwe syntax)
  const subscription = supabase
    ...
    .on(
      ...
      (payload) => {
        console.log('Wine data changed:', payload)
        loadWines().then(() => render())
      }
    )
    .subscribe()
}
...
async function saveWine() {
  ...
  if (editingWineId) {
    ...
    if (error) {
      ...
      alert('Fout bij opslaan in cloud: ' + error.message)
      return
    }
...
async function deleteWine(id) {
  if (confirm('Weet je zeker?')) {
    ...
    if (error) {
      ...
      alert('Fout bij verwijderen uit cloud')
      return
    }
...
`
    ...
    if (score > 0) {
      ...
      const { error } = await supabase
        ...
        .update({
          ai_score: score,
          ai_score_date: wine.ai_score_date
        })
        .eq('id', wine.id)
      
      if (error) {
        console.error('Supabase update AI score error:', error)
      }
      ...
      if (profile) {
        ...
        if (profileWine) {
          ...
          profileWine.aiScoreDate = wine.ai_score_date
          saveProfiles()
        }
      }
    }
...
async function init() {
  await initDB()
  await loadWines()          // Nu van Supabase
  await initSupabaseSync()   // Real-time subscription
  await initLogin()
